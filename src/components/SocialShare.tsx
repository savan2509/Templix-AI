...
import { Share2, Check, Link2, MessageSquare, Send } from "lucide-react";
...
export default function SocialShare({
  ...
  const handleSocialClick = (method: "twitter" | "linkedin" | "email") => {
    if (slug) trackShare(slug, method);
  };
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      <a
        ...
        className="flex h-9 w-9 items-center justify-center rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 transition-all duration-200 hover:scale-105 hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-600 shadow-xs"
      >
        <svg className="h-4 w-4 fill-current" viewBox="0 0 24 24">
      ...
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 h-9 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 text-xs font-bold text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-all shadow-xs"
      >
        {copied ? (
          <>
