...
for (let i = 0; i < lines.length; i++) {
  ...
  if (lines[i].includes('id: "business-proposal"')) {
    // Find the opening brace { of this object
    for (let j = i; j >= 0; j--) {
      if (lines[j].trim() === '{') {
        endIdx = j;
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  const payload = fs.readFileSync('d:\\\\Templix-AI\\\\scratch\\\\contracts_payload.txt', 'utf8');
  ...
  console.log('Replaced contracts fallback templates cleanly!');
} else {
