...
function resolveRedirectPath(inputUrl: string) {
  const parsed = new URL(inputUrl);
  let pathname = parsed.pathname;
  let search = parsed.search;

  // Step 1: Proxy host redirect (platform alias -> https://templix-ai.whitesparksoft.com)
  const canonicalDomain = "https://templix-ai.whitesparksoft.com";

  // Step 2: Proxy locale redirect (es|de|fr|ar -> /en)
  ...
  // Step 5: Specific merged templates
  ...
  // Step 6: Unmapped 2nd/3rd level template filter routes (e.g., /templates/reports/general/canada -> /templates/reports)
  const tplMatch = pathname.match(/^\/en\/templates\/([^\/]+)\/([^\/]+)(?:\/(.*))?$/);
  if (tplMatch) {
    const categorySlug = tplMatch[1];
    const rest = tplMatch[2];
    const extra = tplMatch[3];
    const isCategory = CATEGORIES.some(c => c.slug === categorySlug);
    if (isCategory) {
      const isTemplate = allFallbackTemplates.some(t => t.slug === rest || (extra && t.slug === extra));
      if (!isTemplate && (extra || ["legal", "general", "freelancer"].includes(rest))) {
...
urls.forEach((url, i) => {
  ...
  const targetPath = new URL(resolved).pathname;
  let status = "200 OK Target";

  if (targetPath.startsWith("/en/templates/")) {
    const segs = targetPath.replace("/en/templates/", "").split("/");
    const isCat = CATEGORIES.some(c => c.slug === segs[0]);
    const isTpl = segs[1] ? allFallbackTemplates.some(t => t.slug === segs[1]) : true;
    if (!isCat || !isTpl) status = "INVALID TARGET TEMPLATE";
  } else if (targetPath.startsWith("/en/blog/")) {
    const slug = targetPath.replace("/en/blog/", "");
    const isBlog = STATIC_BLOG_POSTS.some(b => b.slug === slug);
    if (targetPath !== "/en/blog" && !isBlog) status = "INVALID BLOG SLUG";
  } else if (targetPath.startsWith("/en/tools/")) {
    const slug = targetPath.replace("/en/tools/", "");
    const isTool = ALL_TOOLS.some(t => t.slug === slug);
    if (targetPath !== "/en/tools" && !isTool) status = "INVALID TOOL SLUG";
  }
  ...
  if (status.startsWith("200")) {
    ...
    invalidCount++;
    console.log(`[${i + 1}/${urls.length}] ${url} -> ${resolved} (${status})`);
  }
});

console.log(`\nSummary: ${urls.length} total URLs analyzed.`);
console.log(`- Valid 200 OK Targets: ${okCount}`);
console.log(`- Invalid Targets: ${invalidCount}`);
